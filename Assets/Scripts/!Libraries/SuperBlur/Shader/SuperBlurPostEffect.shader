...
Shader "Custom/SuperBlurPostEffect"
{
	...
	fixed3 Desaturate(fixed3 color)
	{
		...
		return lerp(fixed3(lum, lum, lum), color, _Saturation);
#else
		return color;
#endif
	}
	...
	fixed4 frag5Blur (output_5tap IN) : SV_Target
	{
		...
		return fixed4(Desaturate(LinearToGammaSpace(sum)), 1.0);
	#else
		...
		return fixed4(Desaturate(sum), 1.0);
	#endif
	}
	...
	SubShader
	{
		...
		Pass
		{
			...
			#pragma multi_compile _ DESATURATION
